...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // FIX: Server fan accounting - server fans are part of IT load by PUE definition
        ...
        // Test 8.0 (cold) → 100% economizer | 22.0 (mild) → partial | 30.0 (hot) → mechanical
        double testOutdoorTemp_C = 22.0;    // 🔧 CHANGE THIS: 8.0, 22.0, 30.0
        double testOutdoorRH = 40.0;       // 🔧 CHANGE THIS: 40% (dry), 80% (humid)
        
        // ⚙️ Airflow & Fan System Testing  
        double testSupplyFan_W_per_CFM = 0.35;  // 🔧 CHANGE THIS: 0.30 (efficient), 0.45 (design), 0.60 (inefficient)
        double testReturnFan_W_per_CFM = 0.28;  // 🔧 CHANGE THIS: 0.25 (efficient), 0.35 (design), 0.50 (inefficient)
        ...
        // For small systems, use dynamic airflow (425 CFM) rather than oversized fixed approach (912 CFM)
        double smallSystemAirflow_CFM = 425; // Realistic airflow for 3kW system
        double scaledCapex = fixedCapex + (totalITLoadWithFans_kW * baseCapexPerKW) + (smallSystemAirflow_CFM * airflowCapexPerCFM);
        ...
        System.out.printf("IT load-based cost: %.0f kW × $%.0f = $%.0f\n", 
        ...
        System.out.printf("Airflow-based cost: %.0f CFM × $%.1f = $%.0f\n", 
                smallSystemAirflow_CFM, airflowCapexPerCFM, smallSystemAirflow_CFM * airflowCapexPerCFM);
        System.out.printf("Fixed costs: $%.0f (sensors, commissioning)\n", fixedCapex);
        System.out.printf("Scaled CAPEX: $%.0f (right-sized for %.0fkW system)\n", scaledCapex, totalITLoadWithFans_kW);
        ...
        ThermalIntegrator.ThermalAnalysis thermalAnalysis = ThermalIntegrator.analyzeThermalPerformance(racks, supplyTempC);
        ...
        // NOTE: outsideAirFrac_partial and mechTrimFracAtPartial will be set by mode logic below
        ...
        System.out.printf("- Weather: %.1f°C / %.1f%% RH (%s)\n", in.avgOutdoorTemp_C, in.avgOutdoorRH, testCaseLabel);
        System.out.printf("- Airflow: %.0f CFM/kW\n", in.cfmPerKW);
        ...
        // FIX: DYNAMIC AIRFLOW SCALING - Use actual rack demand instead of arbitrary CFM/kW
        ...
        double testTotalCoolingAux_kWh = testFanEnergy_kWh + testMechEnergy_kWh + testPumpEnergy_kWh + testSensorEnergy_kWh;
        
        // ---- Component-wise Energy Calculation (Baseline System) ----
        
        // Baseline HVAC Fans (less efficient)
        double baselineFan_kW = (totalCFM * in.baselineFan_W_per_CFM * 1.10) / 1000.0; // 1.0 W/CFM + 10% penalty
        ...
        double baselineTotalCoolingAux_kWh = baselineFanEnergy_kWh + baselineMechEnergy_kWh + baselinePumpEnergy_kWh + baselineSensorEnergy_kWh;
