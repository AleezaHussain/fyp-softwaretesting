...
import { 
  Save, Trash2, Upload, Download, MapPin, Plus, Settings, Eye, Thermometer, Zap, 
  Wind, Cpu, Database, Power, ChevronRight, Sparkles, Maximize2, Minimize2, 
  Grid3x3, Layers, Target, Bell, Activity, Server, Cloud 
} from 'lucide-react'
...
const Component3D: React.FC<Component3DProps> = ({ type, position, quantity, isSelected, onClick, heatLoad }) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.scale.y = 1 + Math.sin(state.clock.elapsedTime) * 0.1 * (heatLoad / 50)
    }
  })

  const getComponentConfig = () => {
    const configs: Record<string, { color: string; geometry: React.ReactNode; scale: number }> = {
      server_rack: {
  ...
  return (
    <group position={position}>
      ...
      {/* Heat effect */}
      <mesh position={[0, 2, 0]} scale={[1 + (heatLoad / 50), 1 + (heatLoad / 50), 1]}>
        <sphereGeometry args={[0.3, 8, 8]} />
        <meshBasicMaterial color={isDark ? "#ff6b6b" : "#dc2626"} transparent opacity={0.4} />
      </mesh>
...
const DataCenterScene: React.FC<DataCenterSceneProps> = ({ components, selectedId, onSelect }) => {
  const isDark = useThemeStore((state) => state.isDark)
  ...
  return (
    <Canvas shadows camera={{ position: [10, 10, 10], fov: 50 }}>
      ...
      {/* Grid */}
      <gridHelper args={[20, 20, isDark ? '#3f4a68' : '#cbd5e1', isDark ? '#27304a' : '#e2e8f0']} />
...
const DraggableComponent: React.FC<DraggableComponentProps> = ({ component, isSelected, onSelect, onRemove, onUpdate }) => {
  const isDark = useThemeStore((state) => state.isDark)
  ...
  return (
    <Draggable
      bounds="parent"
      defaultPosition={{ x: component.position?.x || 0, y: component.position?.y || 0 }}
      onStop={(_, data) => {
    ...
    >
      <div
        ...
        className={`absolute cursor-grab p-4 rounded-2xl shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-105 ${
          isSelected 
            ? isDark ? 'ring-2 ring-[#5ce1e5] bg-[#1a1f3a]/80' : 'ring-2 ring-[#0ea5e9] bg-white/90'
            : isDark ? 'bg-[#1a1f3a]/60' : 'bg-white/80'
        }`}
        style={{ 
          userSelect: 'none',
          minWidth: '160px',
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="text-sm font-bold capitalize">{component.type.replace('_', ' ')}</div>
        ...
        <div className={`text-xs mb-3 p-2 rounded-lg ${
          ...
          <div className="flex justify-between">
            <span>Heat Load:</span>
            <span className="font-bold">{DEFAULT_HEAT_LOADS[component.type] || 0} kW</span>
          </div>
        </div>
...
const ComponentPaletteItem: React.FC<ComponentPaletteItemProps> = ({ 
  ...
  return (
    <div className={`group p-4 rounded-xl transition-all duration-300 hover:scale-105 ${
      isDark 
...
export const DataCenterBuilder: React.FC = () => {
  const [components, setComponents] = useState<DataCenterComponent[]>([
    { id: uid('rack'), type: 'server_rack', quantity: 1, position: { x: 50, y: 50, z: 0 } },
    { id: uid('router'), type: 'router', quantity: 1, position: { x: 200, y: 50, z: 0 } }
  ])
  ...
  const addComponent = (type: DataCenterComponent['type']) => {
    const newComp: DataCenterComponent = {
      ...
      type,
      quantity: 1,
      position: { x: 100 + Math.random() * 300, y: 100 + Math.random() * 200, z: 0 },
    }
  ...
  const updateComponent = (id: string, updates: Partial<DataCenterComponent>) => {
  ...
  const componentTypes = [
    { type: 'server_rack', name: 'Server Rack', heat: 15, icon: <Cpu className="w-5 h-5" />, color: isDark ? '#fd5757' : '#ef4444' },
    { type: 'router', name: 'Router/Switch', heat: 2, icon: <Zap className="w-5 h-5" />, color: isDark ? '#5ce1e5' : '#0ea5e9' },
    { type: 'cooling_pump', name: 'Cooling System', heat: 3, icon: <Wind className="w-5 h-5" />, color: isDark ? '#10b981' : '#10b981' },
    { type: 'pdu', name: 'Power Distribution', heat: 1, icon: <Power className="w-5 h-5" />, color: isDark ? '#f59e0b' : '#f59e0b' },
    { type: 'storage_array', name: 'Storage Array', heat: 20, icon: <Database className="w-5 h-5" />, color: isDark ? '#8b5cf6' : '#8b5cf6' },
    { type: 'backup_generator', name: 'Backup Generator', heat: 50, icon: <Sparkles className="w-5 h-5" />, color: isDark ? '#f97316' : '#f97316' }
  ]

  return (
    <div ref={containerRef} className={`min-h-screen transition-colors duration-500 ${
      isDark 
      ...
      <div className="lg:ml-64">
        {/* Top Navigation Bar */}
        <div className={`sticky top-0 z-30 p-4 border-b backdrop-blur-sm ${
          isDark 
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className={`p-2 rounded-xl transition-all duration-300 hover:scale-105 ${
                isDark
                  ? 'bg-[#27304a] text-gray-300 hover:bg-[#3f4a68] hover:text-white'
                  : 'bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900'
              }`}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
              <div>
                <h1 className={`text-3xl lg:text-4xl font-bold mb-2 ${
                  ...
                  <span className={isDark ? 'text-[#5ce1e5]' : 'text-[#0ea5e9]'}>Data Center</span> Builder
                </h1>
                <p className={`text-lg ${
                  isDark ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  Design, visualize, and optimize your data center infrastructure
                </p>
              </div>
                ...
                <div className={`flex items-center gap-2 p-1 rounded-xl ${
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg transition-all ${
                      viewMode === '2d'
                        ? isDark ? 'bg-[#5ce1e5] text-white' : 'bg-[#0ea5e9] text-white'
              ...
              <div className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    ...
                    <Thermometer className={`w-6 h-6 ${isDark ? 'text-[#fd5757]' : 'text-[#ef4444]'}`} />
                  </div>
                  <div>
              ...
              <div className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    ...
                    <Zap className={`w-6 h-6 ${isDark ? 'text-[#5ce1e5]' : 'text-[#0ea5e9]'}`} />
                  </div>
                  <div>
                    <div className={`text-2xl font-bold ${
                      isDark ? 'text-white' : 'text-gray-900'
              ...
              <div className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    ...
                    <Download className={`w-6 h-6 ${isDark ? 'text-[#10b981]' : 'text-[#10b981]'}`} />
                  </div>
                  <div>
                    <div className={`text-2xl font-bold ${
                      isDark ? 'text-white' : 'text-gray-900'
              ...
              <div className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 ${
                ...
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${
                    ...
                    <Sparkles className={`w-6 h-6 ${isDark ? 'text-[#8b5cf6]' : 'text-[#8b5cf6]'}`} />
                  </div>
                  <div>
                    <div className={`text-2xl font-bold ${
                      isDark ? 'text-white' : 'text-gray-900'
                ...
                <div className="space-y-4">
                  {componentTypes.map((item) => (
                    <ComponentPaletteItem
                      ...
                      icon={item.icon}
                      color={item.color}
                      onAdd={() => addComponent(item.type as DataCenterComponent['type'])}
                    />
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto">
                  {savedConfigs.map((config, index) => (
                    <div 
                      ...
                      className={`group p-4 rounded-xl transition-all duration-300 hover:scale-105 ${
                        isDark ? 'bg-[#27304a] hover:bg-[#3f4a68]' : 'bg-gray-100 hover:bg-gray-200'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="font-bold">{config.dataCenterName}</div>
                        <div className={`px-2 py-1 rounded text-xs font-bold ${
                          isDark ? 'bg-black/30 text-white' : 'bg-white text-gray-900'
                        }`}>
                  ...
                  {savedConfigs.length === 0 && (
                    <div className={`text-center py-8 ${
                      ...
                      <div className="text-sm">Save your first design to see it here</div>
                    </div>
                  )}
                </div>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className={`inline-block p-4 rounded-2xl ${
                            ...
                            <div className="text-lg font-semibold mb-2">Loading 3D Visualization...</div>
              ...
              <div className={`rounded-2xl p-6 ${
                ...
                {selectedId ? (
                        ...
                        <div className={`p-4 rounded-xl ${
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            <div className={`p-2 rounded-lg ${
                              ...
                              {componentTypes.find(t => t.type === component.type)?.icon || <Cpu className="w-5 h-5" />}
                            </div>
                            <div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`font-bold ${
                                ...
                                {DEFAULT_HEAT_LOADS[component.type] * component.quantity} kW
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-sm font-mono ${
                                ...
                                {component.position?.x || 0}, {component.position?.y || 0}
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className={`text-center py-8 ${
                    ...
                    <div className="text-sm">Click on a component to view and edit its properties</div>
                  </div>
                )}
              </div>
              ...
              <div className={`rounded-2xl p-6 ${
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select className={`w-full px-4 py-3 rounded-xl transition-all ${
                      isDark
                        ? 'bg-[#27304a] text-white border border-[#3f4a68] focus:border-[#5ce1e5]'
                        : 'bg-gray-100 text-gray-900 border border-gray-300 focus:border-[#0ea5e9]'
                    }`}>
