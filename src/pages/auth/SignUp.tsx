...
import {
  Mail, Lock, User, Wind, Zap, Shield,
...
}> = ({ icon: Icon, type, name, value, onChange, placeholder, error,
        showPasswordToggle = false, onTogglePassword, showPassword = false }) => {
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4 gap-8">
        ...
        <div className="hidden lg:flex flex-1 flex-col items-center justify-center max-w-md">
          <div className="space-y-6">
            <div className={`inline-flex items-center gap-3 px-4 py-2 rounded-full ${isDark ? "bg-[#1a1f3a] border border-[#3f4a68]" : "bg-white border border-gray-200"}`}>
              <div className={`p-1.5 rounded-lg ${isDark ? "bg-[#27304a]" : "bg-gray-100"}`}>
                <Sparkles className="w-4 h-4 text-[#5ce1e5]" />
              </div>
              <span className={`text-sm font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>Join the Future of Cooling</span>
            </div>
            ...
            <p className={`text-base ${isDark ? "text-gray-400" : "text-gray-600"}`}>
              Join thousands of data centers optimizing their cooling systems. Save up to 40% on energy costs.
            </p>

            <div className="space-y-2.5">
              {["AI-powered cooling optimization", "Real-time monitoring & alerts", "Detailed analytics & reporting", "Scalable for any data center size"].map((b) => (
                <div key={b} className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${isDark ? "bg-green-500/20" : "bg-green-100"}`}>
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500" />
                  </div>
                  <span className={`text-sm ${isDark ? "text-gray-300" : "text-gray-700"}`}>{b}</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className={`flex items-center gap-2.5 p-3 rounded-xl ${isDark ? "bg-[#1a1f3a] border border-[#3f4a68]" : "bg-white border border-gray-200"}`}>
          ...
          <div className={`relative rounded-3xl overflow-hidden shadow-2xl ${
            ...
            <div className="relative p-6">
              ...
              <div className="text-center mb-4 space-y-2">
                ...
                <h2 className={`text-2xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>Create Account</h2>
                <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-600"}`}>Start optimizing your data center cooling</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <AuthInput icon={User} type="text" name="fullName" value={formData.fullName}
                      onChange={handleChange} placeholder="Full Name" error={errors.fullName} />
                    <AuthInput icon={Mail} type="email" name="email" value={formData.email}
                      onChange={handleChange} placeholder="Email" error={errors.email} />
                  </div>
                  <AuthInput icon={Lock} type="password" name="password" value={formData.password}
                    onChange={handleChange} placeholder="Password" error={errors.password}
                    showPasswordToggle showPassword={showPassword}
                    onTogglePassword={() => setShowPassword((p) => !p)} />
                  <AuthInput icon={Lock} type="password" name="confirmPassword" value={formData.confirmPassword}
                    onChange={handleChange} placeholder="Confirm Password" error={errors.confirmPassword}
                ...
                {formData.password && (
                  <div className={`p-3 rounded-xl ${isDark ? "bg-[#27304a]" : "bg-gray-50 border border-gray-200"}`}>
                    <div className="grid grid-cols-2 gap-1.5">
                      {pwReqs.map(({ label, valid }) => (
                        <div key={label} className="flex items-center gap-1.5">
                          <div className={`w-3 h-3 rounded-full flex items-center justify-center ${valid ? isDark ? "bg-green-500/20" : "bg-green-100" : isDark ? "bg-gray-700" : "bg-gray-200"}`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${valid ? "bg-green-500" : "bg-transparent"}`} />
                          </div>
                          <span className={`text-xs ${valid ? isDark ? "text-green-400" : "text-green-600" : isDark ? "text-gray-500" : "text-gray-400"}`}>{label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {/* Terms */}
                <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? "bg-[#27304a]" : "bg-gray-50 border border-gray-200"}`}>
                  ...
                  <label htmlFor="acceptTerms" className={`text-xs ${isDark ? "text-gray-300" : "text-gray-600"}`}>
                    I agree to the{" "}
                    <Link to="/terms" className={`font-semibold  ${isDark ? "text-[#5ce1e5]" : "text-[#0ea5e9]"}`}>Terms & Conditions</Link>
                    {" "}and{" "}
                    <Link to="/privacy" className={`font-semibold  ${isDark ? "text-[#5ce1e5]" : "text-[#0ea5e9]"}`}>Privacy Policy</Link>
                  </label>
                </div>
                {errors.acceptTerms && <p className="text-red-500 text-xs flex items-center gap-1"><span>⚠</span>{errors.acceptTerms}</p>}
                ...
                <button type="submit" disabled={isSubmitting}
                  ...
                  <span className="relative flex items-center justify-center gap-2">
                    {isSubmitting
                      ...
                      : <><span>Create Account</span><ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>
                    }
                  </span>
                </button>
              </form>
              ...
              <p className={`mt-4 text-center text-base ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                ...
                <Link to="/login" className={`font-semibold ${isDark ? "text-[#5ce1e5] hover:text-[#5ce1e5]/80" : "text-[#0ea5e9] hover:text-[#0ea5e9]/80"} transition-colors`}>
                  Sign in here
                </Link>
              </p>
            </div>
