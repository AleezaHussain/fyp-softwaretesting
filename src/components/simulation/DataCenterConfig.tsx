...
import { DataCenterComponent, ComponentType, ServerRackSize } from '../../types/simulation'
...
export const DataCenterConfig: React.FC<DataCenterConfigProps> = ({
  ...
  const [selectedType, setSelectedType] = useState<ComponentType>('server_rack')
  ...
  const totalComponents = components.reduce((sum, comp) => sum + comp.quantity, 0)
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#1a1a2e] to-[#16213e] rounded-lg p-6 text-white">
        ...
        <p className="text-gray-300">Add components to configure your data center</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <div className="text-4xl font-bold text-[#fd5757]">{totalComponents}</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-sm text-gray-600">Estimated Heat Load</div>
            <div className="text-4xl font-bold text-[#5ce1e5]">{totalHeatLoad.toFixed(1)} kW</div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            ...
            {components.length === 0 ? (
              <div className="p-6 text-center text-gray-400">No components added yet</div>
            ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-[#1a1a2e] text-sm">
                          {COMPONENT_TYPES[comp.type as ComponentType]?.label || comp.type}
                        </div>
        ...
        <div className="lg:col-span-2">
          <DataCenterVisualizer components={components} totalHeatLoad={totalHeatLoad} />
        </div>
      </div>
      ...
      <button
        ...
        className="w-full bg-[#5ce1e5] text-[#1a1a2e] py-3 rounded-lg font-semibold hover:bg-cyan-400 disabled:bg-gray-300 disabled:cursor-not-allowed transition"
      >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-[#1a1a2e] mb-6">Add Component</h3>
            ...
            <div className="space-y-4">
              ...
              {selectedType === 'server_rack' && (
                <div>
                  ...
                  <select
                    value={rackSize}
                    onChange={(e) => setRackSize(e.target.value as ServerRackSize)}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#5ce1e5]"
                  >
