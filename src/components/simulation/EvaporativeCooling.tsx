...
import { EvaporativeWeatherPoint } from "../../services/weatherService";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              ...
              <option value={8760}>Full Year (8760 hours)</option>
            </select>
            <p className={`text-xs mt-1 ${isDark ? "text-gray-500" : "text-gray-500"}`}>
          ...
          {/* Weather Data — Location Picker (auto-fetch from EnergyPlus) */}
          <div className="md:col-span-2">
            <WeatherLocationPicker
              key="evaporative-weather-picker"
              mode="evaporative"
              isDark={isDark}
              onWeatherLoaded={(result) => {
                const points = result.data as EvaporativeWeatherPoint[];
                setWeatherData(points as any);
                setCsvSuccess(true);
                setCsvError(null);
                setWeatherFile({ name: result.city + ".epw" } as any);
              }}
            />
          </div>
