...
const insightRequestCache = new Map<string, Promise<Record<string, GraphExplanation>>>();

const buildInsightCacheKey = (simulationName: string, simulationType: string, sections: InsightSection[], simulationContext: any) => {
    const payload = {
        simulationName,
        simulationType,
        simulationContext,
...
const countBy = (rows: any[], key: string): Record<string, number> => {
    return rows.reduce((acc: Record<string, number>, row) => {
        ...
        const name = rawValue == null || rawValue === "" ? "UNKNOWN" : String(rawValue);
        acc[name] = (acc[name] ?? 0) + 1;
        return acc;
    }, {});
};

const buildSections = (resultData: any, simulationType: string): InsightSection[] => {
    ...
    const economics = resultData?.results?.economics ?? resultData?.economics ?? {};
    ...
    const yearlyData = Array.isArray(resultData?.projection?.yearlyData)
        ...
        : Array.isArray(resultData?.results?.projection?.yearlyData)
            ...
            : [];
    const compTable: any[] = Array.isArray(resultData?.mlRecommendation?.comparison_table)
        ? resultData.mlRecommendation.comparison_table
        : [];
    ...
    if (isChilled) {
        ...
        add(sections, {
            ...
            yAxis: "Value",
            data: [
                { label: "Energy (kWh)", value: pickMetric([annual.energyConsumption_kWh]) },
                { label: "Cooling Load (kWh)", value: pickMetric([annual.coolingLoad_kWh]) },
                { label: "Water (L)", value: pickMetric([annual.waterUsage_L]) },
                { label: "Carbon (kg)", value: pickMetric([annual.carbonEmissions_kg]) },
        ...
        add(sections, {
            ...
            yAxis: "USD",
            data: [
                { label: "Annual OpEx", value: pickMetric([economics.opex_annual_USD, annual.cost_USD]) },
                ...
                { label: "Payback (yrs)", value: pickMetric([economics.paybackPeriod_years]) },
            ],
        });

        const passCount = Object.values(gates).filter((value) => String(value).toUpperCase() === "PASS").length;
        const failCount = Object.values(gates).filter((value) => String(value).toUpperCase() === "FAIL").length;
        add(sections, {
            key: "chilled-gates",
    ...
    if (isAir) {
        ...
        const violations = hourly.filter((row) => row?.airflowViolation).length;
        const pueValues = numbersFrom(hourly, ["pue"]);
        const cueValues = numbersFrom(hourly, ["cue"]);
        const tempValues = numbersFrom(hourly, ["outdoorTempC", "ambientTemp_C", "tempC"]);
        ...
        add(sections, {
            ...
            xAxis: "Mode",
            yAxis: "Hours",
            data: Object.entries(modeBreakdown).map(([mode, hours]) => ({ label: mode.replace(/_/g, " "), value: hours })),
        });
        ...
        add(sections, {
            ...
            yAxis: "USD",
            data: [
                { label: "Electricity Cost", value: pickMetric([summary.electricityCostUSD]) },
                ...
                { label: "Annual Savings", value: pickMetric([summary.annualSavingsUSD]) },
            ],
        });
        ...
        if (rackAnalysis) {
            add(sections, {
                ...
                yAxis: "Value",
                data: [
                    { label: "Average Rack Load", value: pickMetric([rackAnalysis.averageRackLoadKW]) },
                    { label: "Max Rack Load", value: pickMetric([rackAnalysis.maxRackLoadKW]) },
                    { label: "Hotspot Racks", value: pickMetric([rackAnalysis.hotspotRacks]) },
                    { label: "Total Racks", value: pickMetric([rackAnalysis.totalRacks]) },
                ],
            });
        }
    ...
    if (isEvap) {
        ...
        const evapCooling = numbersFrom(evapHourly, ["coolingCapacityKW", "coolingCapacity_kW"]);
        const evapWater = numbersFrom(evapHourly, ["waterEvaporationLph", "waterEvaporation_Lph"]);
        const evapPue = numbersFrom(evapHourly, ["pue"]);
        const evapTemp = numbersFrom(evapHourly, ["ambientTempC", "tempC", "outdoorTempC"]);
        const evapHumidity = numbersFrom(evapHourly, ["supplyHumidity", "humidity", "outdoorRH"]);
        ...
        add(sections, {
            ...
            yAxis: "Value",
            data: [
                { label: "Energy (kWh)", value: pickMetric([pickPath(evapResults, ["energy.electricity_kwh_total"]), annual.energyConsumption_kWh]) },
                { label: "IT Energy (kWh)", value: pickMetric([pickPath(evapResults, ["energy.it_kwh"])]) },
                { label: "Fan Energy (kWh)", value: pickMetric([pickPath(evapResults, ["energy.fan_kwh"])]) },
                { label: "Water (L)", value: pickMetric([pickPath(evapResults, ["water.water_liters_total", "water.water_liters_total"]), evapWater.water_liters_total, annual.waterUsage_L]) },
                { label: "Carbon (kg)", value: pickMetric([pickPath(evapResults, ["emissions.co2_kg_total"]), evapEmissions.co2_kg_total, annual.carbonEmissions_kg]) },
                { label: "Cost (USD)", value: pickMetric([pickPath(evapResults, ["cost.total_energy_cost_usd"]), evapCost.total_energy_cost_usd, annual.cost_USD]) },
                { label: "Avg PUE", value: pickMetric([pickPath(evapResults, ["performance.pue_average"]), evapPerf.pue_average, metrics.pue]) },
                { label: "Avg WUE", value: pickMetric([pickPath(evapResults, ["performance.wue_average"]), evapPerf.wue_average, metrics.wue]) },
            ],
        });
        ...
        add(sections, {
            ...
            data: [
                ...
                { label: "Avg Fan Power", value: avg(numbersFrom(evapHourly, ["fanPowerKW", "fanPower_kW"])) },
                { label: "Avg DX Backup", value: avg(numbersFrom(evapHourly, ["dxPowerKW", "dxPower_kW"])) },
                { label: "Avg Total Electrical", value: avg(numbersFrom(evapHourly, ["totalElectricalKW", "totalElectrical_kW"])) },
            ],
        });
        ...
        add(sections, {
            ...
            data: [
                ...
                { label: "Peak Deficit", value: max(evapPower.map((value, index) => value - (evapCooling[index] ?? 0))) },
                { label: "Avg Water Evaporation", value: avg(evapWater) },
            ],
        });
        ...
        add(sections, {
            ...
            yAxis: "Weather",
            data: [
                { label: "Ambient Temp Avg", value: avg(numbersFrom(evapHourly, ["ambientTempC", "outdoorTempC"])) },
                { label: "Inlet Temp Avg", value: avg(numbersFrom(evapHourly, ["inletTempC"])) },
                { label: "Supply Temp Avg", value: avg(numbersFrom(evapHourly, ["supplyTempC"])) },
                { label: "Ambient Humidity Avg", value: avg(numbersFrom(evapHourly, ["ambientHumidity", "outdoorHumidity"])) },
            ],
        });
        ...
        add(sections, {
            ...
            data: [
                { label: "Supply Temp Avg", value: avg(numbersFrom(evapHourly, ["supplyTempC"])) },
                { label: "Supply Humidity Avg", value: avg(numbersFrom(evapHourly, ["supplyHumidity"])) },
                { label: "Ambient Humidity Avg", value: avg(numbersFrom(evapHourly, ["ambientHumidity"])) },
                { label: "Inlet Temp Avg", value: avg(numbersFrom(evapHourly, ["inletTempC"])) },
            ],
        });
        ...
        add(sections, {
            ...
            data: Object.entries(evapModeCounts)
                ...
                .map(([mode, hours]) => ({ label: mode.replace(/_/g, " "), value: hours })),
        });
        ...
        add(sections, {
            ...
            yAxis: "Value",
            data: [
                { label: "Cooling Capacity Avg", value: pickMetric([km.cooling_capacity_avg_kw]) },
                { label: "Humidity Deviation", value: pickMetric([km.humidity_deviation]) },
    ...
    add(sections, {
        ...
        yAxis: "Score",
        data: [
            { label: "PUE Eff", value: Math.max(0, 10 - ((metrics.pue ?? summary.averagePUE ?? 1.5) - 1) * 5) },
            { label: "CUE Eff", value: Math.max(0, 10 - (metrics.cue ?? summary.averageCUE ?? 0.5) * 10) },
            { label: "Energy Sav", value: Math.min(10, (summary.energySavingsPercent ?? 0) / 5) },
            { label: "Cost Sav", value: Math.min(10, (summary.annualSavingsUSD ?? 0) / 20000) },
            { label: "Carbon Sav", value: Math.min(10, (summary.carbonSavings_kg ?? 0) / 10000) },
        ],
    });

    if (compTable.length > 0) {
        add(sections, {
            key: "shared-comparison",
            title: "Technique Comparison (ML)",
            chartTitle: "Technique Comparison (ML)",
            chartType: "bar",
            xAxis: "Technique",
            yAxis: "Score",
            data: compTable.map((row) => ({
                label: row?.tech ?? row?.technique ?? "Unknown",
                value: pickMetric([row?.score]),
            })),
        });
    }
    ...
    if (yearlyData.length > 0) {
        ...
        add(sections, {
            ...
            yAxis: "Value",
            data: [
                { label: "NPV Savings", value: pickMetric([resultData?.projection?.npvSavings, resultData?.results?.projection?.npvSavings]) },
                { label: "Total Savings", value: pickMetric([resultData?.projection?.totalSavings, resultData?.results?.projection?.totalSavings]) },
                { label: "Total Cost", value: pickMetric([resultData?.projection?.totalCost, resultData?.results?.projection?.totalCost]) },
                { label: "Total Emissions", value: pickMetric([resultData?.projection?.totalEmissions, resultData?.results?.projection?.totalEmissions]) },
                { label: "Avg Yearly Cost", value: avg(yearlyEnergyCost) + avg(yearlyCarbonTax) },
    ...
    return sections.filter((section) => section.data.length > 0 && section.data.some((point) => point.value !== 0));
};

export const SimulationChartsInsightPanel: React.FC<SimulationChartsInsightPanelProps> = ({
    simulationName,
    simulationType,
    ...
    const sections = useMemo(() => buildSections(resultData, simulationType), [resultData, simulationType]);
    ...
    const simulationContext = useMemo(() => {
        const summary = resultData?.summary ?? resultData?.results?.summary ?? {};
        return {
            location: summary.location ?? resultData?.location ?? simulationDescription ?? simulationName,
            ...
            workloadPercent: toNumber(summary.workloadPercent, resultData?.workloadPercent),
    ...
    const panelFetchKey = useMemo(() => {
        return buildInsightCacheKey(simulationName, simulationType, sections, simulationContext);
    ...
    const fetchInsights = async (forceRefresh = false) => {
        ...
        try {
            ...
            if (storedInsights) {
                const parsedInsights = JSON.parse(storedInsights) as Record<string, GraphExplanation>;
        ...
        if (cachedRequest) {
            ...
            try {
                ...
                setError(err instanceof Error ? err.message : "Failed to generate chart explanations");
            } finally {
                setLoading(false);
            }
            return;
        ...
        const requestPromise = (async () => {
            ...
            if (failedSections.length > 0) {
                setError(`Some chart explanations could not be generated: ${failedSections.join(", ")}`);
            }
    ...
    if (sections.length === 0) {
        return (
            <div className={`rounded-xl border p-5 ${isDark ? "bg-[#1a1f3a] border-[#3f4a68]" : "bg-white border-gray-200"}`}>
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between gap-3">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Sparkles className={`w-5 h-5 ${isDark ? "text-[#5ce1e5]" : "text-blue-600"}`} />
            ...
            {error && (
                <div className={`flex items-start gap-2 rounded-lg border px-3 py-2 text-sm ${isDark ? "border-red-900/50 bg-red-950/30 text-red-200" : "border-red-200 bg-red-50 text-red-700"}`}>
            ...
            {loading && (
                <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${isDark ? "border-[#3f4a68] bg-[#0f1530] text-gray-300" : "border-gray-200 bg-gray-50 text-gray-600"}`}>
                        ...
                        >
                                ...
                                {section.data.map((point) => (
                                    ...
                                    >
                                        ...
                                        <p className={`text-sm font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>
                                            {Number.isFinite(point.value) ? point.value.toLocaleString() : "0"}
                                        </p>
                                    </div>
                                ))}
                            </div>
                            ...
                            {insight ? (
                                <div className="space-y-2">
                                    ...
                                    {insight.keyInsight && (
                                        <div className={`rounded-md border-l-4 px-3 py-2 ${isDark ? "border-[#5ce1e5] bg-[#1a1f3a]" : "border-blue-500 bg-white"}`}>
                                            <p className={`text-[11px] font-semibold uppercase tracking-wide ${isDark ? "text-[#5ce1e5]" : "text-blue-700"}`}>
                                                Quick Takeaway
                                            </p>
