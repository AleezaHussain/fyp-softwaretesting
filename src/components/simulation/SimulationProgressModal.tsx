...
export const SimulationProgressModal: React.FC = () => {
  ...
  const simulationFailureReason = useSimulationStore(
    (s) => (s as any).simulationFailureReason as string | null,
  );
  const isDark = useThemeStore((s) => s.isDark);
  const setSimulationRunning = useSimulationStore(
    (s) => s.setSimulationRunning,
  );
  const setSimulationProgress = useSimulationStore(
    (s) => s.setSimulationProgress,
  );
  const setSimulationStatus = useSimulationStore((s) => s.setSimulationStatus);
  const setSimulationFailureReason = useSimulationStore(
    (s) => s.setSimulationFailureReason,
  );
  ...
  const clearProgressState = useCallback(() => {
    ...
    setSimulationStatus("");
    setSimulationFailureReason(null);
    setCurrentSimulation(null);
  }, [
    setCurrentSimulation,
    setSimulationFailureReason,
  ...
  const currentStatusKey = isFailed
    ? "failed"
    : isCanceled
      ? "cancelled"
      : isComplete
  ...
  const Toast = completionToast && (
    ...
    >
      ...
      <button
        ...
        className="ml-auto text-lg leading-none opacity-60 hover:opacity-100 shrink-0"
      >
        ×
      </button>
    </div>
  ...
  if (!isSimulationRunning && simulationProgress === 0 && !completionToast)
    return null;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        ...
        >
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              >
                {isFailed ? (
                  ...
                  <span className="text-yellow-500">Simulation Cancelled</span>
                ) : isComplete ? (
                  <span className="text-green-500">Simulation Complete!</span>
                ) : (
                  <>
                    <span style={{ color: statusMeta.color }}>Running</span>{" "}
                    Simulation
                  </>
                )}
              </h3>
          ...
          <div className="p-8 pb-16">
            ...
            {/* Status text */}
            <p
              className={`text-center mb-2 text-lg font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}
            >
              {isCanceled
            ...
            {isFailed && simulationFailureReason && (
              <div
                className={`mt-2 mb-4 px-4 py-3 rounded-xl text-sm border ${
                  isDark
            ...
            <div className="mb-4">
              <div
                className={`w-full h-3 rounded-full overflow-hidden ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
              >
                <div
            ...
            {!isFailed && !isCanceled && (
              <div className="space-y-2 mt-6">
                ...
                <ProgressStep
                  ...
                  isActive={
                    simulationProgress >= 90 && simulationProgress < 100
                  }
                />
              </div>
            )}
