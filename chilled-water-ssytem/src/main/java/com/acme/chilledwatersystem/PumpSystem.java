...
public class PumpSystem {
    ...
    /**
     * Calculate pump electrical power (kW) for a design flow rate and current IT
     * load.
    ...
    public double calculate(double designFlowLps, double itLoadKW, double designMaxLoadKW) {
        ...
        double fraction = 0.0;
        if (designMaxLoadKW > 0)
            fraction = Math.max(0.05, Math.min(1.0, itLoadKW / designMaxLoadKW));
