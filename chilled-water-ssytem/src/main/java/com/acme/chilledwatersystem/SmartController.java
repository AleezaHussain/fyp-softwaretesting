 ...
 * - Decides economizer/free-cooling availability based on wet-bulb & supply
 * setpoint.
 * - Suggests CRAH fan speed fraction (0..1) using cubic-law vs IT load.
 * - Nudges chilled-water supply temp setpoint up/down within ASHRAE envelope to
 * save energy.
 */
public class SmartController {
    ...
    public Decision decide(double itLoadKW, double wetBulbC, double currentInletAirC) {
        ...
        // Economizer: permit if wet-bulb is sufficiently below CHW set (simple
        // heuristic)
        d.economizerOn = (wetBulbC + 3.0) <= chwSupplySetC;

        // CRAH fan fraction: scale with IT load up to a nominal capacity (500 kW
        // reference)
        double frac = Math.max(0.15, Math.min(1.0, itLoadKW / 500.0));
        d.crahFanFrac = frac;

        // Adaptive CHW setpoint: if inlet air comfortably < max, raise CHW; if close to
        // max, lower a bit
    ...
    // Tuners
    public void setChwMaxSetC(double v) {
        this.chwMaxSetC = v;
    }

    public void setChwMinSetC(double v) {
        this.chwMinSetC = v;
    }

    public void setChwSupplySetC(double v) {
        this.chwSupplySetC = v;
    }
}
