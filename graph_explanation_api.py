...
def _extract_json_candidates(text: str) -> List[str]:
    """Return balanced JSON-object substrings from a response string."""
    candidates: List[str] = []
...
def _parse_llm_payload(response_text: str) -> Optional[LLMGraphPayload]:
    """Try multiple strategies to extract structured payload from model output."""
    ...
    try:
        ...
        return LLMGraphPayload(**parsed)
    except Exception:
        pass

    # 2) Parse balanced JSON object candidates and choose the most relevant one
    ...
    for candidate in reversed(candidates):
        ...
        if isinstance(parsed, dict) and ("explanation" in parsed or "key_insight" in parsed):
            try:
                return LLMGraphPayload(**parsed)
            except Exception:
                continue
...
def _strip_embedded_json(text: str) -> str:
    """Remove embedded JSON blocks from explanation text if the model echoed payload."""
    cleaned = text.strip()
    if not cleaned:
        return cleaned
    ...
    with _GROQ_REQUEST_LOCK:
        ...
        for attempt in range(4):
            try:
                ...
                payload = _parse_llm_payload(response_text)
                if payload is None:
                    payload = LLMGraphPayload(explanation=response_text, key_insight=None)

                payload.explanation = _strip_embedded_json(payload.explanation)
                if not payload.explanation:
                    payload.explanation = _strip_embedded_json(response_text)
