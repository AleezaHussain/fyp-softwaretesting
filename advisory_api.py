...
def _provider_chain() -> list[str]:
    allowed = {"groq", "openrouter", "gemini"}
    configured = [p.strip().lower() for p in LLM_PROVIDER_CHAIN.split(",") if p.strip()]
    if configured:
        return [p for p in configured if p in allowed]

    if LLM_PROVIDER in allowed:
        return [LLM_PROVIDER]
...
def health() -> Dict[str, Any]:
    ...
    return {
        ...
        "model": OPENROUTER_MODEL,
        "active_model": (
            GROQ_MODEL if active == "groq" else GEMINI_MODEL if active == "gemini" else OPENROUTER_MODEL
        ),
